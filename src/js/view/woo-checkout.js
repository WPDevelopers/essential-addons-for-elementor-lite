var WooCheckout = function ($scope, $) {
   ...
   $(document.body).on("removed_coupon_in_checkout", function () {
      let message = $(
         ".ea-woo-checkout .ms-tabs-content > .woocommerce-message,.ea-woo-checkout .split-tabs-content > .woocommerce-message",
      ).remove();
      $(".ea-woo-checkout .checkout_coupon.woocommerce-form-coupon").before(
         message,
      );
   });
   ...
   $(document).on(
      "change",
      ".eael-checkout-cart-qty-input",
      ea.debounce(eael_update_checkout, 300),
   );
};
   ...
   jQuery(document.body).on(
      ...
      function (event, country, wrapper) {
         let $ = jQuery,
            ...
            field_wrapper = $(
               ".ea-woo-checkout .woocommerce-billing-fields__field-wrapper, .ea-woo-checkout .woocommerce-shipping-fields__field-wrapper",
            );
         ...
         let reorder_fields = function (type, _wrapper) {
            ...
            $.each(checkout_keys[type], function (field_key, form_class) {
               ...
               if ($fieldHtml.length === 0) {
                  $fieldHtml = _wrapper
                     .find(`input[name='${field_key}']`)
                     .closest("p");
               }
               $fieldHtml
                  .removeClass("form-row-first form-row-last form-row-wide")
                  .addClass(form_class);
               $(
                  `#eael-wc-${type}-reordered-fields .eael-woo-${type}-fields`,
               ).append($fieldHtml);
            });
            $selector.replaceWith(
               $(`#eael-wc-${type}-reordered-fields`).contents(),
            );
            $(`.eael-woo-${type}-fields`).toggleClass(
               `eael-woo-${type}-fields woocommerce-${type}-fields__field-wrapper`,
            );
            $(`#eael-wc-${type}-reordered-fields`).html(
               `<div class="eael-woo-${type}-fields"></div>`,
            );
         };
...
jQuery(document.body)
   .on(
      "update_checkout payment_method_selected updated_checkout",
      function (event) {
         change_button_text();
      },
   )
