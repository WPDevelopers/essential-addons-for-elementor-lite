...
| Shop Header | header | Simple Menu, Creative Button | Light bar: brand left, navigation centred, then search and cart discs and a sign-in button. Collapses to a hamburger at tablet; the button steps out at mobile |
...
**A preset declares every widget type it can emit**, in a `widgets` key, and is dropped from the list whenever one of them is missing. This is not belt-and-braces: `document/elements/create` builds the tree depth first and throws the moment it meets an unknown widget type, and *what it already built stays on the canvas* — so a missing widget turns one click into a half-inserted preset plus an error toast. Not offering the card is the only clean answer.

`Elements::has_widget()` asks `Plugin::$instance->widgets_manager->get_widget_types()` rather than testing for a class or an experiment, because a widget can be missing for four unrelated reasons — Elementor too old, an experiment off, the element switched off in EA's own settings, or deactivated in Elementor's element manager — and the widgets manager is the one answer that covers all four. Third-party presets added through the filter may declare `widgets` too; one that omits the key is simply not gated.
...
**Creative Button has a 150px floor.** Its stylesheet sets `min-width: 150px` with no control behind it, so a short label — `Shop_Header`'s "Login" — sits in a button wider than its padding asks for, and on a phone that is a third of the screen. That is why the sign-in steps out at mobile there. Core's Button hugs its label instead, which is the swap to make when the width matters more than the widget.
...
**Option values are enums, and Elementor 4 renamed several of them.** A `CHOOSE` or `SELECT` value that is not one of the control's declared options still *renders* — many of these controls carry a back-compat `selectors_dictionary` that maps the old word to the new CSS — but the panel shows no option selected, so the user cannot see or adjust what the preset chose. Alignment is the trap: `heading`, `text-editor`, `image`, `icon` and `icon-list` moved to `start` / `center` / `end`, `icon-box`'s icon position to `inline-start` / `block-start`, and `button`'s icon position to `row` / `row-reverse` — while `button`'s own alignment and `social-icons` still use `left` / `center` / `right`. The values also matter for RTL: `text-align: start` flips, `text-align: left` does not.

The same applies to setting *names*: an unknown key is stored and silently ignored. Info Box is the one to watch — the only typography group for its body copy is named `eael_infobox_content_typography_hover`, despite styling the normal state.

Both classes of mistake are invisible on screen, so they are worth checking mechanically against `elementor.widgetsCache[ type ].controls` and `elementor.config.elements.container.controls` — the same stacks the panel reads. Note that a widget's cache does *not* include the Advanced-tab controls (`_title`, `hide_mobile`, `_margin`…); those live in `elementor.widgetsCache.common.controls` and have to be merged in, or every preset will look like it is full of unknown settings.
