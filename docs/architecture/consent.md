...
- **Disclosure copy (the "What we collect" content):** hardcoded in [`Helper::start_plugin_tracking()` lines 137-141](../../includes/Traits/Core.php#L137):
  ...
  - **Extra notice (the slide-open "What we collect"):** rewritten in **6.7.2** to enumerate every field actually transmitted — site URL/name, WP/PHP/server version, language, charset, text direction, **active** plugin list, inactive plugin *count*, active theme + version, EA element usage, and admin email for the coupon — plus explicit statements that nothing is sent before consent, that visitor data / content / inactive plugin names are never collected, and that consent can be withdrawn. The previous copy ("non-sensitive diagnostic data … plugins & themes") understated the payload; see [`wpinsights.md`](wpinsights.md) for the field-by-field table. Keep this copy in sync with `get_data()` whenever the payload changes.
