...
class Formstack extends Widget_Base {
	...
	private function no_app_setup() {
		...
		$this->add_control(
			...
			array(
				...
				'raw'             => __(
					'Please set your app client credentials on the <strong>Formstack</strong> settings page.',
	...
	private function no_forms_created() {
		...
		$this->add_control(
			...
			array(
				'type'            => Controls_Manager::RAW_HTML,
				'raw'             => __(
					'Please create form on the <strong>Formstack</strong> settings page.',
					'essential-addons-for-elementor-lite'
				),
				'content_classes' => 'eael-warning',
			)
	...
	private function formstack_not_activated() {
		...
		$this->add_control(
			...
			array(
				'type'            => Controls_Manager::RAW_HTML,
				'raw'             => __(
					'<strong>Formstack Online Forms</strong> is not installed/activated on your site. Please install and activate <a href="plugin-install.php?s=formstack&tab=search&type=term" target="_blank">Formstack Online Forms</a> first.',
					'essential-addons-for-elementor-lite'
				),
				'content_classes' => 'eael-warning',
			)
	...
	protected function get_forms() {
		...
		$keys = array( '' => __( '-- Select One --', 'essential-addons-for-elementor-lite' ) );
		...
		$this->add_control(
			...
			array(
				...
				'description' => sprintf(
					...
					sprintf(
						'<a href="%s">%s</a>',
						esc_url( add_query_arg( array( 'clear_formstack_cache' => 'true' ), admin_url( 'admin.php?page=Formstack' ) ) ),
	...
	protected function render() {
		...
		<div <?php $this->print_render_attribute_string( 'eael_formstack_wrapper' ); ?>>
			...
			<div class="fsForm">
				<?php
                // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped, WordPress.Security.EscapeOutput.OutputNotEscaped
				echo $this->parse_text_editor( eael_neutralize_shortcodes( wp_kses( $form_data, Helper::eael_allowed_tags() ) ) );
				?>
			</div>
		</div>
		<?php
