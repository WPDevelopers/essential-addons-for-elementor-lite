        ...
        <div class="eael-nft-item <?php echo esc_attr( $pagination_class ); ?> ">
		        ...
		        if ( $nft_gallery['show_thumbnail'] ) {
			        if ( ! empty( $item_formatted['thumbnail'] ) ) {
				        if ( $nft_gallery['thumbnail_clickable'] && 'preset-1' === $nft_gallery['preset'] ) {
					        printf( '<a href="%s" target="_blank" >', eael_neutralize_shortcodes( esc_url( $item_formatted['view_details_link'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ].
				        }

				        printf( '<img src="%s" alt="%s">', eael_neutralize_shortcodes( esc_attr( $item_formatted['thumbnail'] ) ), esc_attr__( 'NFT Gallery', 'essential-addons-for-elementor-lite' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ].
            ...
            <div class="eael-nft-main-content">
                ...
                <div class="eael-nft-content">
                    ...
                    <h3 class="eael-nft-title"><?php printf('%s', eael_neutralize_shortcodes( esc_html( $item_formatted['title'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></h3>
                    <?php endif; ?>
                    ...
                    <div class="eael-nft-current-price-wrapper">
                        ...
                        <p class="eael-nft-current-price"><?php printf('%s %s', floatval( $item_formatted['current_price'] / $unit_convert ), eael_neutralize_shortcodes( esc_html( $item_formatted['currency'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></p>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="eael-nft-creator-wrapper">
                        ...
                        <div class="eael-nft-created-by">
                            ...
                            <div><?php printf('<a target="_blank" href="%s">%s</a>', eael_neutralize_shortcodes( esc_attr( $item_formatted['created_by_link'] ) ), eael_neutralize_shortcodes( esc_html( $item_formatted['created_by_link_text'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></div>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="eael-nft-owner-wrapper">
                        <div class="eael-nft-owner-img">
                            ...
                            if (!empty($item_formatted['owner_thumbnail'])) {
                                printf('<img src="%s" alt="%s">', eael_neutralize_shortcodes( esc_url( $item_formatted['owner_thumbnail'] ) ), esc_attr__('EA NFT Owner Thumbnail', 'essential-addons-for-elementor-lite') ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ].
                        ...
                        <div class="eael-nft-owned-by">
                            ...
                            <div><?php printf('<a target="_blank" href="%s">%s</a>', eael_neutralize_shortcodes( esc_url( $item_formatted['owned_by_link'] ) ), eael_neutralize_shortcodes( esc_html( $item_formatted['owned_by_link_text'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></div>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div class="eael-nft-last-sale-wrapper">
                        <?php if( intval($item_formatted['last_sale']) > 0 ): ?>
                            <p class="eael-nft-last-sale"><?php printf('<span class="eael-nft-last-sale-text">%s</span> <span class="eael-nft-last-sale-price">%s %s</span>', esc_html($nft_gallery['last_sale_label'] ), floatval($item_formatted['last_sale'] / $unit_convert ), eael_neutralize_shortcodes( esc_html( $item_formatted['currency'] ) )); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></p>
                        <?php elseif( ! empty($item_formatted['ends_in']) ): ?>
                            <p class="eael-nft-ends-in"><?php printf('<span class="eael-nft-ends-in-text">%s</span> <span class="eael-nft-ends-in-time">%s</span>' , esc_html($nft_gallery['ends_in_label'] ), eael_neutralize_shortcodes( esc_html( $item_formatted['ends_in'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></p>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="eael-nft-button">
                    ...
                    <button <?php $this->print_render_attribute_string('eael-nft-gallery-button'); ?>>
                        <?php printf('<a target="_blank" href="%s">%s</a>', eael_neutralize_shortcodes( esc_attr( $item_formatted['view_details_link'] ) ), esc_html( $nft_gallery['view_details_text'] ) ) // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?>
                    </button>
                    <?php endif; ?>
                </div>
            </div>
	        ...
	        if ( $nft_gallery['thumbnail_clickable'] && 'preset-2' === $nft_gallery['preset'] ) {
		        printf( '<a href="%s" target="_blank" ></a>', eael_neutralize_shortcodes( esc_url( $item_formatted['view_details_link'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ].
	        }
	        ?>
        </div>
        <?php 
            ...
            <div class="eael-nft-main-content">
                ...
                <div class="eael-nft-content eael-nft-grid-container">
                    ...
                    <div class="eael-nft-title-wrapper eael-nft-grid-item">
                        <h3 class="eael-nft-title"><?php printf('<a href="%s" target="_blank">%s</a>', eael_neutralize_shortcodes( esc_url( $item_formatted['view_details_link'] ) ), eael_neutralize_shortcodes( esc_html( $item_formatted['title'] ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped inside; eael_neutralize_shortcodes() only encodes [ and ]. ?></h3>
                    </div>
                    <?php endif; ?>
            ...
            <div class="eael-nft-gallery-loadmore-wrap">
                <a href="#" <?php $this->print_render_attribute_string('nft-gallery-load-more-button'); ?>>
                    ...
                    <?php if ($settings['eael_nft_gallery_button_icon_position'] == 'before') { ?>
                        <?php if ($icon_is_new || $icon_migrated) { ?>
                            <?php if (isset($settings['eael_nft_gallery_load_more_icon_new']['value']['url'])) : ?>
                                <img class="eael-nft-gallery-load-more-icon nft-gallery-load-more-icon-left" src="<?php echo esc_url($settings['eael_nft_gallery_load_more_icon_new']['value']['url']); ?>" alt="<?php echo esc_attr(Helper::get_image_alt( $settings['eael_nft_gallery_load_more_icon_new']['value'] )); ?>" />
                            <?php else : ?>
                    ...
                    <?php if ($settings['eael_nft_gallery_button_icon_position'] == 'after') { ?>
                        <?php if ($icon_is_new || $icon_migrated) { ?>
                            <?php if (isset($settings['eael_nft_gallery_load_more_icon_new']['value']['url'])) : ?>
                                <img class="eael-nft-gallery-load-more-icon nft-gallery-load-more-icon-right" src="<?php echo esc_url($settings['eael_nft_gallery_load_more_icon_new']['value']['url']); ?>" alt="<?php echo esc_attr(Helper::get_image_alt( $settings['eael_nft_gallery_load_more_icon_new']['value'] )); ?>" />
                            <?php else : ?>
