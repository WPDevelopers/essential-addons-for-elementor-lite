...
echo ob_get_clean();
    ...
    protected function _content_template() {
        ?>
            function eael_infobox_before() {
                view.addRenderAttribute( 'eael_infobox_inner', {
                    'class': [
                        'eael-infobox'
                    ],
                } );
            ...
            var $target = settings.eael_show_infobox_clickable_link.is_external != '' ? 'target="_blank"' : '';
            var $target = settings.eael_show_infobox_clickable_link.nofollow != '' ? 'rel="nofollow"' : '';
        }

    function render_infobox_icon() {

        if ( 'none' == settings.eael_infobox_img_or_icon ) {
            return;
        }

        var infobox_image = settings.eael_infobox_image;
        var infobox_image = {
            id: settings.eael_infobox_image.id,
            url: settings.eael_infobox_image.url,
            model: view.getEditModel()
        };
        var flipbox_image_url = elementor.imagesManager.getImageUrl( infobox_image );

        view.addRenderAttribute( 'infobox_icon', {
            'class': [
                'infobox-icon'
            ],
        } );
        ...
        if ( settings.eael_infobox_icon_hover_animation ) {
            view.addRenderAttribute( 'infobox_icon', {
                'class': [
                    'elementor-animation-' + settings.eael_infobox_icon_hover_animation
                ],
            } );
        }
        ...
        if (settings.eael_infobox_image_icon_hover_animation) {
            view.addRenderAttribute( 'infobox_icon', {
                'class': [
                    'elementor-animation-' + settings.eael_infobox_image_icon_hover_animation
                ],
            } );
        }
        ...
        if (settings.eael_infobox_number_icon_hover_animation) {
            view.addRenderAttribute( 'infobox_icon', {
                'class': [
                    'elementor-animation-' + settings.eael_infobox_number_icon_hover_animation
                ],
            } );
        }
        ...
        var iconHTML = elementor.helpers.renderIcon( view, settings.eael_infobox_icon_new, { 'aria-hidden': true }, 'i' , 'object' ),
            migrated = elementor.helpers.isIconMigrated( settings, 'eael_infobox_icon' );

        var $icon_tag = "";
        if ( settings.eael_infobox_icon_new || settings.eael_infobox_icon ) {
            var $icon = settings.eael_infobox_icon_new;

            if ( $icon.url !== undefined ) {
                view.addRenderAttribute( 'icon_or_image', {
                    'src' => $icon['url'],
                    'alt' => ''
                } );
                
                <!-- var $icon_tag = '<img {{{ view.getRenderAttributeString( 'icon_or_image' ) }}} />'; -->
            } else {
                view.addRenderAttribute( 'infobox_icon', {
                    'class': $icon,
                } );
            ...
            <!-- var $icon_tag = '<i class="{{{ settings.eael_infobox_icon }}}"></i>'; -->
        }
        <div {{{ view.getRenderAttributeString( 'infobox_icon' ) }}}>

            <# if ( 'img' == settings.eael_infobox_img_or_icon ) { #>
                <img src="{{{ $infobox_image_url }}}" />
            <# } #>

            <# if ('icon' == settings.eael_infobox_img_or_icon ) { #>
                <div class="infobox-icon-wrap">{{{ $icon_tag }}}</div>
            <# } #>

            <# if ('number' == settings.eael_infobox_img_or_icon ) { #>
                <div class="infobox-icon-wrap">
                    <span class="infobox-icon-number">{{{ settings.eael_infobox_number }}}</span>
                </div>
            <# } #>
