...
class Woo_Product_Tabs extends Widget_Base {
	...
	protected function render() {
		...
		add_filter( 'woocommerce_product_tabs', array( $this, 'manage_product_tabs' ), 98 );
		...
		remove_filter( 'woocommerce_product_tabs', array( $this, 'manage_product_tabs' ), 98 );
	...
	private function inject_tab_icons( $html ) {
		...
		$settings = $this->get_settings_for_display();
		$items    = ! empty( $settings['eael_product_tabs_items'] ) ? $settings['eael_product_tabs_items'] : array();
		...
		foreach ( $items as $item ) {
			...
			$icon      = isset( $item['eael_product_tabs_item_icon'] ) ? $item['eael_product_tabs_item_icon'] : array();
	...
	private function get_tab_icon_html( $icon ) {
		...
		$icon_html = Icons_Manager::try_get_icon_html( $icon, array( 'aria-hidden' => 'true' ) );
