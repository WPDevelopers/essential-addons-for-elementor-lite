...
function App() {
	...
	useEffect(() => {
		eaState.isDark
			? document.body.classList.add("eael_dash_dark_mode")
			: document.body.classList.remove("eael_dash_dark_mode");
	...
	useEffect(() => {
		...
		const flagMap = [
			...
			{
				flags: ["eael_pinterest_success", "eael_pinterest_disconnected", "eael_pinterest_error"],
				modalID: "pinterestFeedSetting",
				title: "Pinterest Feed",
			},
		];

		const match = flagMap.find((entry) => entry.flags.some((flag) => urlParams.get(flag) === "1"));
	...
	return (
		<>
			...
			<section id="ea__dashboard--wrapper" className="ea__dashboard--wrapper" ref={wrapperRef}>
				...
				<section
					className={
						eaState.menu === "Elements"
							? "ea__section-wrapper ea__main-wrapper flex"
							: "ea__section-wrapper ea__main-wrapper flex gap-4"
					}
				>
					<Menu />
