    ...
    public function twitter_feed_render_items($id, $settings, $class = '')
    {
        ...
        foreach ($items as $item) {
            ...
            if ($settings['eael_twitter_feed_show_read_more'] == 'true') {
                $html .= '<a href="//twitter.com/' . @$item['user']['screen_name'] . '/status/' . $item['id_str'] . '" target="_blank" class="read-more-link">' . __('Read More', 'essential-addons-for-elementor-lite') . ' <i class="fas fa-angle-double-right"></i></a>';
            }
            $html .= '</div>
