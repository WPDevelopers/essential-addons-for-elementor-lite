	...
	public function twitter_feed_render_items( $id, $settings, $class = '' ) {
		...
		if ( $items === false ) {
			...
			if ( $twitter_v2 ) {
				...
				$tweet_fields        = array( 'entities', 'public_metrics', 'in_reply_to_user_id', 'attachments', 'created_at' );
		...
		foreach ( $items as $item ) {
			...
			if ( ! empty( $settings['eael_twitter_feed_hash_linked'] ) && $settings['eael_twitter_feed_hash_linked'] === 'yes' && ! empty( $item['entities']['hashtags'] ) ) {
				...
				foreach ( $item['entities']['hashtags'] as $hashtag ) {
					...
					if ( $hashtag['text'] ) {
						$hashtags[ '#' . esc_html( $hashtag['text'] ) ] = "<a href='https://twitter.com/hashtag/" . esc_attr( $hashtag['text'] ) . "?src=hashtag_click' target='_blank'>#" . esc_html( $hashtag['text'] ) . '</a>';
					}
				}
			...
			if ( $twitter_v2 ) {
				$item['id_str']                    = $item['id'];
				$item['entities']['user_mentions'] = ! empty( $item['entities']['mentions'] ) ? $item['entities']['mentions'] : array();
			}
			...
			if ( ! empty( $settings['eael_twitter_feed_mention_linked'] ) && $settings['eael_twitter_feed_mention_linked'] === 'yes' && $item['entities']['user_mentions'] ) {
				...
				foreach ( $item['entities']['user_mentions'] as $mention ) {
					...
					if ( $mention['screen_name'] ) {
						$mentions[ '@' . esc_html( $mention['screen_name'] ) ] = "<a href='https://twitter.com/" . esc_attr( $mention['screen_name'] ) . "' target='_blank'>@" . esc_html( $mention['screen_name'] ) . '</a>';
					}
				}
