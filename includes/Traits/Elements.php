	...
	public function render_global_html() {
		...
		if ( $this->get_settings( 'smooth-animation' ) ) {
			...
			foreach ( $event_gorups as $event_gorup_item ) {
				...
				$event_markers              = $event_gorup_item['eael_smooth_animation_event_markers'];
				$event_canvas_scrub         = ! empty( $event_gorup_item['eael_smooth_animation_event_canvas_scrub']['size'] ) ? $event_gorup_item['eael_smooth_animation_event_canvas_scrub']['size'] : '1';
				...
				$event_transform_scale      = ! empty( $event_gorup_item['eael_smooth_animation_event_scale']['size'] ) ? $event_gorup_item['eael_smooth_animation_event_scale']['size'] : '';
				...
				$event_canvas_loop          = $event_gorup_item['eael_smooth_animation_event_loop'];
				$event_trans_originx          = $event_gorup_item['eael_smooth_animation_trans_originx'];
				$event_trans_originy          = $event_gorup_item['eael_smooth_animation_trans_originy'];

				//
				$event_transform_translatex_unit          = $event_gorup_item['eael_smooth_animation_event_transform_translatex']['unit'];
				$event_transform_translatey_unit          = $event_gorup_item['eael_smooth_animation_event_transform_translatey']['unit'];

				print_r($event_markers);
				?>
					<script>
					gsap.registerPlugin(ScrollTrigger);
						...
						<?php
							...
							$eael_smooth_animation_obj = "gsap.{$event_function}('{$event_target}', {";
							if (!empty($event_transform_translatex)) {
								$eael_smooth_animation_obj .= " x: '{$event_transform_translatex}{$event_transform_translatex_unit}',";
							}
							if (!empty($event_transform_translatey)) {
								$eael_smooth_animation_obj .= " y: '{$event_transform_translatey}{$event_transform_translatey_unit}',";
							}
							if (!empty($event_trans_originx & $event_trans_originy)) {
								$eael_smooth_animation_obj .= " transformOrigin: '{$event_trans_originx} {$event_trans_originy}',";
							}
							if (!empty($event_transform_scale)) {
								$eael_smooth_animation_obj .= " scale: '{$event_transform_scale}',";
							}
							if (!empty($event_transform_rotate)) {
								$eael_smooth_animation_obj .= "rotate: '{$event_transform_rotate}',";
							}
							if (!empty($event_canvas_duration)) {
								$eael_smooth_animation_obj .= "duration: '{$event_canvas_duration}',";
							}
							// if (!empty($event_canvas_loop)) {
							// 	$eael_smooth_animation_obj .= "repeat: '{$event_canvas_loop}',";
							// }
							if (!empty($event_animation_easing)) {
								$eael_smooth_animation_obj .= "ease: '{$event_animation_easing}',";
							}
								$eael_smooth_animation_obj .= "scrollTrigger: {";
									if (!empty($event_target)) {
										$eael_smooth_animation_obj .= "trigger: '{$event_target}',";
									}
									if (!empty($event_toggle_actions)) {
										$eael_smooth_animation_obj .= "toggleActions: '{$event_toggle_actions}',";
									}
									if (!empty($event_canvas_start)) {
										$eael_smooth_animation_obj .= "start: '{$event_canvas_start}',";
									}
									if (!empty($event_canvas_end)) {
										$eael_smooth_animation_obj .= "end: '{$event_canvas_end}',";
									}
									if (!empty($event_canvas_scrub)) {
										$eael_smooth_animation_obj .= "scrub: '{$event_canvas_scrub}',";
									}
									if (!empty($event_pin_element)) {
										$eael_smooth_animation_obj .= "pin: '{$event_pin_element}',";
									}
									if (!empty($event_markers)) {
										$eael_smooth_animation_obj .= "markers: '{$event_markers}',";
									}
