...
trait Woo_Product_Comparable {
	...
	public function init_style_content_controls( $css_classes = array() ) {
		extract( $css_classes );
	...
	public function init_style_table_controls( $css_classes = array() ) {
		extract( $css_classes );
		...
		$this->add_control(
			...
			array(
				...
				'separator' => 'before',
				'selectors' => array( $table_title => 'color:{{VALUE}}' ),
	...
	public function init_style_table_common_style( $tbl = '' ) {
		...
		$this->add_control(
			...
			array(
				...
				'return_value' => 'yes',
				'separator'    => 'before',
				'condition'    => array( 'separate_col_style!' => 'yes' ),
			)
		);
		$this->start_popover();
		$this->add_responsive_control(
			...
			array(
				...
				'condition'  => array( 'common_th_style_pot' => 'yes' ),
			)
		);
		$this->add_responsive_control(
			'table_gen_th_padding',
			array(
		...
		$this->add_responsive_control(
			...
			array(
				...
				'condition'  => array( 'common_td_style_pot' => 'yes' ),
			)
		);
		$this->add_responsive_control(
			'table_gen_td_padding',
			array(
	...
	public function init_style_product_column_style( $column_number, $tbl = '' ) {
		...
		/*-----HOVER state------ */
		$this->start_controls_tab(
			"{$pfx}_tab_style_hover",
			array(
				'label' => __( 'Hover', 'essential-addons-for-elementor-lite' ),
			)
		);
		$this->add_control(
			"{$pfx}_btn_color_hover",
			array(
				'label'     => __( 'Button Text Color', 'essential-addons-for-elementor-lite' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => array( $btn_hover => 'color:{{VALUE}}' ),
	...
	public function init_style_compare_button_controls( $condition = null ) {
		...
		$this->start_controls_tab( 'compare_btn_style_tab_normal', array( 'label' => esc_html__( 'Normal', 'essential-addons-for-elementor-lite' ) ) );
		...
		$this->add_group_control(
			...
			array(
				...
				'selector'  => '{{WRAPPER}} .eael-product-grid .woocommerce li.product .button.eael-wc-compare',
				'condition' => array(
					'eael_product_grid_style_preset' => array( 'eael-product-default', 'eael-product-simple' ),
				),
			)
		);
		...
		$this->start_controls_tab( 'compare_btn_hover_tab', array( 'label' => esc_html__( 'Hover', 'essential-addons-for-elementor-lite' ) ) );
	...
	public static function render_compare_table( $options ) {
		...
		$icon                   = ! empty( $ds['field_icon'] ) && ! empty( $ds['field_icon']['value'] ) ? $ds['field_icon'] : array();
		...
		<?php
		do_action( 'eael/wcpc/after_content_wrapper' );
	}
					...
					switch ( $field ) {
						...
						default:
							if ( taxonomy_exists( $field ) ) {
								...
								do_action(
									'eael/wcpc/compare_field_' . $field,
									array(
										$product,
										&$product->fields,
									)
	...
	public function fields( $products = array() ) {
		...
		if ( empty( $fields ) || ! is_array( $fields ) ) {
			return apply_filters( 'eael/wcpc/products_fields_none', array() );
		}
