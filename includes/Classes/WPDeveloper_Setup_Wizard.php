...
class WPDeveloper_Setup_Wizard {
	...
	public function setup_wizard_scripts( $hook ) {
		if ( isset( $hook ) && $hook == 'admin_page_eael-setup-wizard' ) {
			...
			wp_localize_script(
				'essential_addons_elementor-setup-wizard-js',
				'localize',
				array(
	...
	public function tab_step() {
		...
		<ul class="eael-quick-setup-wizard <?php echo esc_attr( $wizard_column ); ?>" data-step="1">
			<?php foreach ( $items as $item ) : ?>
				<?php
				if ( $item == 'Templately' && $this->templately_status || ( $this->get_local_plugin_data( 'templately/templately.php' ) !== false && $item == 'Templately' ) ) {
					continue;}
				?>
	...
	public function templately_integrations() {
		...
		<div id="templately" class="eael-quick-setup-tab-content templately setup-content"
			...
			<div class="eael-quick-setup-title">
				<?php
				printf(
					__( '%1$s <span class="eael-quick-setup-highlighted-red">%2$s</span> %3$s', 'essential-addons-for-elementor-lite' ),
					'Unlock',
					'2000+',
					'Ready Templates Built With Elementor & Essential Addons From
                    Templately.'
				);
				?>
			</div>
			<div class="eael-quick-setup-text">
				<?php
				_e(
					'Get Access to amazing features and boost your Elementor page
                    building experience with Templately 👇',
					'essential-addons-for-elementor-lite'
				);
				?>
			</div>
