...
if ( has_post_thumbnail() ) {
	...
	$settings['eael_image_size_customize_size'] = $settings['eael_woo_product_list_image_size_size'];
	$thumbnail_html = Group_Control_Image_Size::get_attachment_image_html( $settings,'eael_image_size_customize' );
}

$title_tag = isset( $settings['eael_woo_product_list_title_html_tag'] ) ? Helper::eael_validate_html_tag($settings['eael_woo_product_list_title_html_tag'])  : 'h2';
$should_print_compare_btn = isset( $settings['show_compare'] ) && 'yes' === $settings['show_compare'];

if ( function_exists( 'YITH_WCWL' ) ) {
	$should_print_wishlist_btn = isset( $settings['eael_woo_product_list_wishlist'] ) && 'yes' === $settings['eael_woo_product_list_wishlist'];
}
// Improvement
$list_style_preset = isset($settings['eael_woo_product_list_style_preset']) ? $settings['eael_woo_product_list_style_preset'] : '';
...
// should print vars
$should_print_rating = isset( $settings['eael_woo_product_list_rating'] ) && 'yes' === $settings['eael_woo_product_list_rating'];
$should_print_quick_view = isset( $settings['eael_woo_product_list_quick_view'] ) && 'yes' === $settings['eael_woo_product_list_quick_view'];
$should_print_image_clickable = isset( $settings['eael_woo_product_list_image_clickable'] ) && 'yes' === $settings['eael_woo_product_list_image_clickable'];
$should_print_price = isset( $settings['eael_woo_product_list_price'] ) && 'yes' === $settings['eael_woo_product_list_price'];
$should_print_excerpt = isset( $settings['eael_woo_product_list_excerpt'] ) && ('yes' === $settings['eael_woo_product_list_excerpt'] && has_excerpt());
...
if ( $list_style_preset == 'eael-product-simple' || $list_style_preset == 'eael-product-reveal' ) { ?>
    <li class="product">
        <div class="eael-product-wrap">
            ...
            <?php echo wp_kses_post( $product->get_image( $settings['eael_woo_product_list_image_size_size'], [ 'loading' => 'eager' ] ) );
            if ( $should_print_image_clickable ) {
	            echo '</a>';
            }
            ...
            if ( $should_print_compare_btn ) {
                Woo_Product_List::print_compare_button( $product->get_id() );
            }
            ?>
            <?php
    ...
    <?php
} else if ( $list_style_preset == 'eael-product-overlay' ) {
    ?>
    <li <?php post_class( 'product' ); ?>>
        <div class="overlay">
            ...
            echo $product->get_image( $settings['eael_woo_product_list_image_size_size'], [ 'loading' => 'eager' ] );
            if ( $should_print_image_clickable ) {
	            echo '</a>';
            }
            ?>
            <div class="button-wrap clearfix">
                ...
                woocommerce_template_loop_add_to_cart();
                if ( $should_print_compare_btn ) {
                    Woo_Product_List::print_compare_button( $product->get_id(), 'icon' );
                }
                ?>
	            <?php
	            if ( ! empty( $should_print_wishlist_btn ) ) {
    ...
    <?php
} else if (($list_style_preset == 'eael-product-preset-5') || ($list_style_preset == 'eael-product-preset-6') || ($list_style_preset == 'eael-product-preset-7')) {
            ...
            <div class="eael-product-wrap">
                <div class="product-image-wrap">
                    ...
                    <div class="image-hover-wrap">
                        <?php if($list_style_preset == 'eael-product-preset-5'){ ?>
                                ...
                                <?php if( $should_print_quick_view ){?>
                                    <li class="eael-product-quick-view">
                                        <a id="eael_quick_view_<?php echo uniqid(); ?>" data-quickview-setting="<?php echo htmlspecialchars(json_encode($quick_view_setting),ENT_QUOTES); ?>"
                                           class="eael-woo-product-list-open-popup open-popup-link">
                        ...
                        <?php } elseif ($list_style_preset == 'eael-product-preset-7') { ?>
                ...
                <div class="product-details-wrap">
                    <?php
                    if(($list_style_preset == 'eael-product-preset-7') && $should_print_price ){
                        echo '<div class="eael-product-price">'.$product->get_price_html().'</div>';
                    }
                    ...
                    <?php if(($list_style_preset != 'eael-product-preset-7') && $should_print_price ){
                        echo '<div class="eael-product-price">'.$product->get_price_html().'</div>';
                    }?>
                </div>
            </div>
...
} else if ($list_style_preset == 'eael-product-preset-8') {
    if ( true === wc_get_loop_product_visibility( $product->get_id() ) || $product->is_visible() ) {
        ?>
        <li <?php post_class( 'product' ); ?>>
            <div class="eael-product-wrap">
                ...
                <div class="product-details-wrap">
                    ...
                    if ($list_style_preset == 'eael-product-list-preset-2') {
                        ...
                        if ( $should_print_excerpt ) {
                            ...
                            echo '<p>' . wp_trim_words(strip_shortcodes(get_the_excerpt()), $settings['eael_woo_product_list_excerpt_length'], $settings['eael_woo_product_list_excerpt_expanison_indicator']) . '</p>';
                            echo '</div>';
                        }
                        if ( $should_print_price ) {
                            echo '<div class="eael-product-price">'.$product->get_price_html().'</div>';
                        ...
                        if ( $should_print_excerpt ) {
                            ...
                            echo '<p>' . wp_trim_words(strip_shortcodes(get_the_excerpt() ? get_the_excerpt() :
                                    get_the_content()), $settings['eael_woo_product_list_excerpt_length'], $settings['eael_woo_product_list_excerpt_expanison_indicator']) . '</p>';
                            echo '</div>';
                        }
                        echo '</div>';
    ...
    if ( $should_print_compare_btn ) {
        add_action( 'woocommerce_after_shop_loop_item', [
            '\Essential_Addons_Elementor\Elements\Woo_Product_List',
            'print_compare_button',
        ] );
    }

    $thumb_size = isset($settings['eael_woo_product_list_image_size_size']) ? $settings['eael_woo_product_list_image_size_size'] : '';
