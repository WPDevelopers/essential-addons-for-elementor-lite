...
class Image_Masking {
	...
	/**
	 * Strip characters that could break out of the <style> block or inject
	 * new CSS rules/declarations from a user-supplied CSS fragment.
	 */
	private static function sanitize_css_fragment( $value ) {
	...
	public function __construct() {
		...
		add_filter( 'elementor/document/element/replace_id', array( $this, 'cleanup_settings_data' ) );
		add_filter( 'elementor/document/save/data', array( $this, 'cleanup_settings_data' ) );
	}
	...
	public function enqueue_scripts() {
		$data = array( 'svg_dir_url' => self::$svg_dir_url );
	...
	private function masking_controllers( $element, $tab = '' ) {
		...
		$element->add_control(
			...
			array(
				...
				'condition'    => array_merge( $condition, array( 'eael_image_masking_enable_custom_clip_path' . $tab . '!' => 'yes' ) ),
			)
		);
		...
		$condition = array( 'eael_image_masking_type' => 'image' );
		if ( '_hover' === $tab ) {
		...
		if ( ! apply_filters( 'eael/pro_enabled', false ) ) {
			$element->add_control(
				...
				array(
					...
					'condition'       => array_merge( $condition, array( 'eael_image_masking_svg' . $tab => 'upload' ) ),
				)
			);
		} else {
			do_action( 'eael/image-masking/image_control', $element, array_merge( $condition, array( 'eael_image_masking_svg' . $tab => 'upload' ) ), $tab );
		}
	...
	public function before_render( $element ) {
		...
		if ( 'yes' === $is_enabled ) {
			...
			if ( 'clip' === $type ) {
				...
				if ( 'yes' === $settings['eael_image_masking_hover_effect'] ) {
					...
					if ( $hover_clip_path_value ) {
						...
						if ( $hover_selector ) {
							$hover_selector = ' ' . self::sanitize_css_fragment( trim( $hover_selector ) );
						}
			...
			} elseif ( 'morphing' === $type ) {
				$morphing_options = apply_filters( 'eael/image_masking/morphing_options', array(), $element, $element_id );
