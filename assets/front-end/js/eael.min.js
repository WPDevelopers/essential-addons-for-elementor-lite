...
var filterableGalleryHandler=function(s,f){var l,a,t,e=s.find(".fg-layout-3-filter-controls").eq(0),i=s.find("#fg-filter-trigger"),n=s.find(".fg-layout-3-search-box"),o=s.find("#fg-search-box-input");if(n.length&&n.on("submit",function(e){e.preventDefault()}),i.on("click",function(){e.toggleClass("open-filters")}).blur(function(){e.toggleClass("open-filters")}),!isEditMode){var g=f(".eael-filter-gallery-container",s),r=g.data("settings"),c=g.data("gallery-items"),d="masonry"==r.grid_style?"masonry":"fitRows",u="yes"==r.gallery_enabled,m=g.isotope({itemSelector:".eael-filterable-gallery-item-wrap",layoutMode:d,percentPosition:!0,stagger:30,transitionDuration:r.duration+"ms",filter:function(){var e=f(this),t=!l||e.text().match(l);null==a&&(a=s.find(".eael-filter-gallery-control ul li").first().data("filter"));var i=!a||e.is(a);return t&&i}});s.on("click",".control",function(){var e=f(this);a=f(this).attr("data-filter"),s.find("#fg-filter-trigger > span")&&s.find("#fg-filter-trigger > span").text(e.text()),e.siblings().removeClass("active"),e.addClass("active"),m.isotope()}),o.on("input",function(){var e=f(this);clearTimeout(t),t=setTimeout(function(){l=new RegExp(e.val(),"gi"),m.isotope()},600)}),m.imagesLoaded().progress(function(){m.isotope("layout")}),m.on("arrangeComplete",function(){m.isotope("layout")}),f(window).on("load",function(){m.isotope("layout")}),f(".eael-magnific-link",s).magnificPopup({type:"image",gallery:{enabled:u},callbacks:{close:function(){f("#elementor-lightbox").hide()}}}),f(s).magnificPopup({delegate:".eael-magnific-video-link",type:"iframe",callbacks:{close:function(){f("#elementor-lightbox").hide()}}}),s.on("click",".eael-gallery-load-more",function(e){e.preventDefault();var t=f(this),i=f(".eael-filter-gallery-container",s).children(".eael-filterable-gallery-item-wrap").length,l=g.data("total-gallery-items"),a=g.data("images-per-page"),n=g.data("nomore-item-text"),o=[];i==l&&(t.html('<div class="no-more-items-text">'+n+"</div>"),setTimeout(function(){t.fadeOut("slow")},600));for(var r=i;r<i+a;r++)o.push(f(c[r])[0]);g.append(o),m.isotope("appended",o),m.imagesLoaded().progress(function(){m.isotope("layout")}),f(".eael-magnific-link",s).magnificPopup({type:"image",gallery:{enabled:u},callbacks:{close:function(){f("#elementor-lightbox").hide()}}})})}};jQuery(window).on("elementor/frontend/init",function(){elementorFrontend.hooks.addAction("frontend/element_ready/eael-filterable-gallery.default",filterableGalleryHandler)});
