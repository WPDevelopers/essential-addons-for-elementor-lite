...
var filterableGalleryHandler=function(s,f){var a,l,t,e=s.find(".fg-layout-3-filter-controls").eq(0),i=s.find("#fg-filter-trigger"),o=s.find("#fg-search-box-input");if(i.on("click",function(){e.toggleClass("open-filters")}).blur(function(){e.toggleClass("open-filters")}),!isEditMode){var g=f(".eael-filter-gallery-container",s),n=g.data("settings"),c=g.data("gallery-items"),r="masonry"==n.grid_style?"masonry":"fitRows",d="yes"==n.gallery_enabled,u=g.isotope({itemSelector:".eael-filterable-gallery-item-wrap",layoutMode:r,percentPosition:!0,stagger:30,transitionDuration:n.duration+"ms",filter:function(){var e=f(this),t=!a||e.text().match(a),i=!l||e.is(l);return t&&i}});s.on("click",".control",function(){var e=f(this);l=f(this).attr("data-filter"),s.find("#fg-filter-trigger > span")&&s.find("#fg-filter-trigger > span").text(e.text()),e.siblings().removeClass("active"),e.addClass("active"),u.isotope()}),o.on("input",function(){var e=f(this);clearTimeout(t),t=setTimeout(function(){a=new RegExp(e.val(),"gi"),u.isotope()},600)}),u.imagesLoaded().progress(function(){u.isotope("layout")}),u.on("arrangeComplete",function(){u.isotope("layout")}),f(window).on("load",function(){u.isotope("layout")}),f(".eael-magnific-link",s).magnificPopup({type:"image",gallery:{enabled:d},callbacks:{close:function(){f("#elementor-lightbox").hide()}}}),f(s).magnificPopup({delegate:".eael-magnific-video-link",type:"iframe",callbacks:{close:function(){f("#elementor-lightbox").hide()}}}),s.on("click",".eael-gallery-load-more",function(e){e.preventDefault();var t=f(this),i=f(".eael-filter-gallery-container",s).children(".eael-filterable-gallery-item-wrap").length,a=g.data("total-gallery-items"),l=g.data("images-per-page"),o=g.data("nomore-item-text"),n=[];i==a&&(t.html('<div class="no-more-items-text">'+o+"</div>"),setTimeout(function(){t.fadeOut("slow")},600));for(var r=i;r<i+l;r++)n.push(f(c[r])[0]);g.append(n),u.isotope("appended",n),u.imagesLoaded().progress(function(){u.isotope("layout")}),f(".eael-magnific-link",s).magnificPopup({type:"image",gallery:{enabled:d},callbacks:{close:function(){f("#elementor-lightbox").hide()}}})})}};jQuery(window).on("elementor/frontend/init",function(){elementorFrontend.hooks.addAction("frontend/element_ready/eael-filterable-gallery.default",filterableGalleryHandler)});
